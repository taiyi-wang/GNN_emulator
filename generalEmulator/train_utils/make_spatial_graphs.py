...
### Run this file to make the fixed spatial graphs ###
	...
	if use_parallel == False:
		...
		for n in range(n_grids):
			...
			if extend_grids == True:
				...
				offset_x[0, 0] += extend1
				offset_x[0, 1] += extend2
				scale_x[0, 0] += extend3
				scale_x[0, 1] += extend4
			...
			pos_grid = kmeans_packing_focused(scale_x, offset_x, 3, n_nodes_grid, n_batch = 10000, n_steps = 8000, n_sim = 1, lr = 0.005)[0]/scale_up # .to(device)
		...
		pos_grid_l = kmeans_packing_focused_parallel(num_cores, scale_x_list, offset_x_list, 3, n_nodes_grid, n_batch = 10000, n_steps = 8000, n_sim = 1, lr = 0.005) # [0] # /scale_up
		for n in range(n_grids):
...
if (make_grid == True)*(save_grid == True):
	...
	np.savez_compressed('Grids/spatial_grid_logarithmic_heterogenous_ver_%d.npz'%save_grid_ver, pos_grid_l = [pos_grid_l[j] for j in range(len(pos_grid_l))])
...
if ext_type == 'local':
	np.savez_compressed('Grids/cayley_grid_heterogenous_ver_%d.npz'%save_grid_ver, A_edges_c = A_edges_c)