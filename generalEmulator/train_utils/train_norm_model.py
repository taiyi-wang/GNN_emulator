# Training the model that predicts the amplitude of the displacement field
...
m = GNN_Network_Norm_Mesh_Enhanced(device = device).to(device)
...
if n_restart is not None:
	...
	m.load_state_dict(torch.load(path_to_model + 'trained_heterogenous_norm_model_step_%d_ver_%d.h5'%(n_restart, n_ver_save)))
	optimizer.load_state_dict(torch.load(path_to_model + 'trained_heterogenous_norm_model_optimizer_step_%d_ver_%d.h5'%(n_restart, n_ver_save)))
...
for i in range(n_begin, n_epochs):
	...
	pos_slice, signal_slice, edges_slice, trgt_slice = load_batch_data_norm_mesh_enhanced(st_files, shape_vals, params)
	...
	if np.mod(i, n_vald_steps) == 0:
		...
		# with open(path_to_model + 'output_norm_%d.txt'%n_ver_save, 'a') as text_file:
		# 	text_file.write('%d loss %0.9f, %0.9f \n'%(i, losses[-1], losses_vald[-1]))

	if np.mod(i, n_save_steps) == 0:

		torch.save(m.state_dict(), path_to_model + 'trained_heterogenous_norm_model_step_%d_ver_%d.h5'%(i, n_ver_save))
		torch.save(optimizer.state_dict(), path_to_model + 'trained_heterogenous_norm_model_optimizer_step_%d_ver_%d.h5'%(i, n_ver_save))
		np.savez_compressed(path_to_model + 'trained_heterogenous_norm_model_prediction_step_%d_ver_%d.npz'%(i, n_ver_save), pred = pred.cpu().detach().numpy(), trgt = trgt_batch.cpu().detach().numpy(), losses = losses, losses_vald = losses_vald, itrain = itrain, ivald = ivald)